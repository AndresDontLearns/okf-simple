...
from okf.tools.context import clear_web_state, set_web_state
from okf.tools.web_tools import fetch_url
from okf.web.fetcher import Page
...
def test_seed_fetch_succeeds_and_records_link_depth():
    _set_state()
    with patch(
        "okf.tools.web_tools.fetch_and_parse",
        return_value=_page(
...
def test_fetch_url_without_web_state():
    clear_web_state()
    with patch(
        "okf.tools.web_tools.fetch_and_parse",
        return_value=_page(
        ...
        result = fetch_url("https://docs.example.com/docs/intro")
    mock_fetch.assert_called_once_with("https://docs.example.com/docs/intro")
    assert "error" not in result
    assert result["url"] == "https://docs.example.com/docs/intro"
    assert result["title"] == "t"
    assert result["markdown"] == "m"
    assert result["links"] == ["https://docs.example.com/docs/next"]
